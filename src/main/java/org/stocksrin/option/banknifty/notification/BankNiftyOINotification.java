...
public class BankNiftyOINotification {
	...
	private static int negitiveOIDiffLit = 200000;
	...
	private static String getDate() {
		return DateUtils.getCurrentDate() + DateUtils.getCurrentMonth().toUpperCase() + DateUtils.getCurrentYear();
	}
			...
			for (OptionModle optionModle : result) {
				...
				for (OptionModle da : data) {
					if (optionModle.getStrike_price().equals(da.getStrike_price())) {
						...
						if (optionModle.getC_change_oi() != null && !optionModle.getC_change_oi().equals(da.getC_change_oi())) {
							callOINotification(expiry, optionModle, da);
						}

						if (optionModle.getP_change_oi() != null && !optionModle.getP_change_oi().equals(da.getP_change_oi())) {
