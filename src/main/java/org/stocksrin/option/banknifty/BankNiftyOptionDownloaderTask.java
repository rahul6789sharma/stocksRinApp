...
public class BankNiftyOptionDownloaderTask extends TimerTask {
	...
	public static List<String> getdataFaced() throws Exception {
		...
		OptionModles columns = getBankNiftyOptionChain(APPConstant.BANKNIFTY_WEEKLY_OPTION_URL);
	...
	private static List<String> otherExpiry(List<String> expiryList) throws Exception {
		...
		for (int i = 0; i < size; i++) {
			...
			OptionModles columns = getBankNiftyOptionChain(url);
			OptionModles optionModles = bankNiftyOptionDataWrapper(columns);
	...
	private static OptionModles bankNiftyOptionDataWrapper(OptionModles columns) {
		return columns;
/*		OptionModles optionModles = new OptionModles();
		List<OptionModle> colums = columns.getOptionModle();
		List<OptionModle> lst = new ArrayList<>();

		for (OptionModle column : colums) {
	...
	private static OptionModles getBankNiftyOptionChain(String url) {
